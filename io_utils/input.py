...
class Input:
    ...
    def __init__(self, inputfile):
        ...
        self.save_load_split_flag = self.set_para("save_load_split_flag", "False", bool)
        ...
        self.range_low = self.set_para("range_low", None, float)
        self.range_high = self.set_para("range_high", None, float)

        if self.range_low is not None and self.range_high is not None:
            assert math.isclose(
                self.range_low, self.range_high
            ), f"the lower bound {self.range_low} must be less than {self.range_high}"
