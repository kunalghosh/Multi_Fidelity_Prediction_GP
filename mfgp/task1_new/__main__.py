...
from io_utils import overwrite, append_write, out_condition, out_time, out_time_all, out_time_all_temp, fig_atom, fig_HOMO
import datetime
import time
...
#    kernel = ConstantKernel(constant_value = const , constant_value_bounds=(1e-5, 1e5)) * RBF(length_scale=length, length_scale_bounds=(1e-5, 1e5)) # best result.
#    const = 13.0606
#    length =  702.3319
#    kernel = ConstantKernel(constant_value = const , constant_value_bounds=(1e-5, 1e5)) * RBF(length_scale=length, length_scale_bounds=(1e-5, 1e5)) # best result.
#    kernel = ConstantKernel(constant_value = const , constant_value_bounds=(const*1e-1, const*1e1)) * RBF(length_scale=length, length_scale_bounds=(length*1e-1, length*1e1)) # best result.
...
#    with open('para_bef_0.txt', 'w') as f1:
#        print(para_bef, file=f1)
#    with open('para_aft_0.txt', 'w') as f2:
...
#        kernel = ConstantKernel(constant_value = const, constant_value_bounds=(const*1e-1, const*1e1)) * RBF(length_scale = length, length_scale_bounds=(length*1e-1, length*1e1)) # best result.
        kernel = ConstantKernel(constant_value = const, constant_value_bounds=(const*1.0/bound, const*bound)) * RBF(length_scale = length, length_scale_bounds=(length*1.0/bound, length*bound)) # best result.
        #    gpr = GaussianProcessRegressor( kernel = kernel, optimizer = None,normalize_y = normalize_y, n_restarts_optimizer=0)            
        gpr = GaussianProcessRegressor( kernel = kernel, normalize_y = normalize_y, n_restarts_optimizer = n_opt)            
...
#        length = para_kernel_aft['k2__length_scale'] 
#        kernel = ConstantKernel(constant_value = const, constant_value_bounds = (1e-5, 1e5)) * RBF(length_scale = length, length_scale_bounds=(1e-5, 1e5)) # best result.
#        kernel = ConstantKernel(constant_value = const, constant_value_bounds=(const*1e-2, const*1e2)) * RBF(length_scale = length, length_scale_bounds=(length*1e-2, length*1e2)) # best result.
#        gpr = GaussianProcessRegressor( kernel = kernel, normalize_y = normalize_y, n_restarts_optimizer=0)
